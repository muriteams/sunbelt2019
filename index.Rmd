...
\raggedleft We will focus on the first one
...
- Which completed 1 hour of group tasks (collective intelligence developed by MIT [@Kim2017])\pause

- Survey capturing information regarding socio-demographics **and**:\pause
    
    - \uscred{Social Intelligence} (SI domains): Social Perception (measured by RME), Social Accomodation, Social Gregariousness, and Social Awareness
...
We can do a lot of simple statistics: density, prop of \textit{\color{gray}[blank]}, etc. but...\pause{} \uscred{\large how can we go beyond that?}
...
ERGMs can do the job, but the only problem is...\pause{} have you tried estimating ERGMs in small networks?
...
-  Sometimes we also go for using a single big (very sparse) graph\pause
    ...
    -  Block diagnoal matrix 
    
    -  Constrain the sampling space puting structural zeros (thanks statnet for the `blockdiag(attrname)` constraint!)\pause
...
*   What if our networks have at most that (6 nodes)? \pause
...
-  Enough flexibility to, e.g., go for pooled estimates (helps with \uscred{power} \textit{and} \uscred{degeneracy})\pause

-  Pooled estimates is a natural approach for small networks \pause

-  Generate random sequences of ERGMs from fitted models fast (in-memory) \pause
...
*  Obtain estimates \uscred{faster}, and \pause

*  obtain \uscred{more accurate} estimates.
...
2.  Using those parameters, sampled $n_1\sim\mbox{Poisson}(10), n_2\sim\mbox{Poisson}(10), n_3\sim\mbox{Poisson}(10)$
    networks of size 3, 4, and 5 respectively.

3.  Estimated the pooled ERGMs using both the MLE and the bootstrap version.

## Simulation study: Empirical Bias
...
*   This is not new.\pause{} What's new is the set of tools to apply it\pause

*   Taking this approach we can improve our estimates (power) and help with
    degeneracy (all by pooling)\pause

*   The tool is working (that's what the simulation say)\pause

*   Need to conduct more simulations using _nodal_ attributes\pause
...
-  More flexible formulas (e.g. interactions between terms)\pause
...
\Large \uscred{Exact Statistics and Semi-Parametric Tests for Small Network Data}
...
We still care about the relation between collective intelligence and performance.\pause{} How can we assess this?\pause

-  GLMs and alike\pause

-  Permutation tests\pause

-  Use fitted ERGM to our benefit, e.g. sample from ERGM distribution to
   do a simulation based test.\pause
   
## Pseudo-bonus track (contd')

(sorry the heavy notation, but) Suppose that we have a 3 networks of sizes 4, 4, and 5 respectively. 
