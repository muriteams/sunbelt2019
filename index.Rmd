...
- Which completed 1 hour of group tasks (collective intelligence developed by our collaborators at MIT)\pause
...
-  In general, we are interested on assessing the following: $\left.\left(\graph\perp\depvar\right) \right| \indepvar$ ?\pause

-  Ways to solve this: parametrically (e.g. GLMs) and non-parametrically (permutation tests):\pause

    -  Parametrically: Sample size?\pause
...
*   Need to conduct more simulations using _nodal_ attributes and compare with ERGM block diagnoal models.\pause
...
-  Simulation based methods (small size $\implies$ sampling from in-memory data)\pause
